  ...
  return (
    <div
      ...
      className={clsx(
        ...
        isBlueBackground
          ? 'hover:bg-text-invert hover:border-text-invert'
          : 'hover:border-text-primary  hover:bg-text-primary'
      )}
    >
      {isDark ? (
        <SunIcon isBlueBackground={isBlueBackground} />
      ) : (
        <MoonIcon isBlueBackground={isBlueBackground} />
      )}
    </div>
  );
}

const SunIcon = ({ isBlueBackground }: { isBlueBackground: boolean }) => (
  <svg
...
const MoonIcon = ({ isBlueBackground }: { isBlueBackground: boolean }) => (
  <svg
    className="w-[1.6rem] h-[1.6rem]"
    viewBox="0 0 17 17"
    fill="none"
